...
import com.nvpacademy.Phucacademy.model.*;
import com.nvpacademy.Phucacademy.reponse.LoginReponse;
import com.nvpacademy.Phucacademy.reponsitory.PersonRepository;
import com.nvpacademy.Phucacademy.service.AuthService;
import com.nimbusds.jose.*;
import com.nimbusds.jose.crypto.MACSigner;
import com.nimbusds.jose.crypto.MACVerifier;
import com.nimbusds.jwt.JWTClaimsSet;
import com.nimbusds.jwt.SignedJWT;
import com.nvpacademy.Phucacademy.request.RefreshTokenRequest;
...
public class AuthenticationRestController {
    ...
    // @PostMapping("/login")
    // public ResponseEntity<AuthenticationResponse> login(@RequestBody LoginReponse
    // person, HttpServletResponse response){
    ...
    @PostMapping("/login")
    public ResponseEntity<AuthenticationResponse> login(@RequestBody LoginReponse person,
        ...
        if (!user.isPresent()) {
            ...
            if (person.isCheckedRememberMe()) {
                ...
                response.addHeader("Set-Cookie",
                        "RefreshJWTToken=" + refreshToken + "; Max-Age=" + maxAgeRefresh + "; Path=/; HttpOnly");
            } else {
    ...
    public ResponseEntity<AuthenticationResponse> loginWithService(HttpSession session, @RequestBody Person person,
            HttpServletResponse response) {
        String email = person.getEmail();
        ...
        if (user.isPresent()) {
            ...
            // Set RefreshJwt Cookie
            response.addHeader("Set-Cookie",
                    "RefreshJWTToken=" + refreshToken + "; Expires=" + expires + "; Path=/; HttpOnly");
            AuthenticationResponse authenticationResponse = new AuthenticationResponse(token, refreshToken, true);
            return ResponseEntity
                    .status(HttpStatus.OK)
                    .body(authenticationResponse);
    ...
    @PostMapping("/refresh")
    public void refresh(HttpServletResponse response, @RequestParam("checkedRemember") boolean checkedRemember,
            @CookieValue(value = "RefreshJWTToken", required = false) String refreshTokenCookie,
            @RequestBody(required = false) RefreshTokenRequest requestBody)
            throws JOSEException, ParseException, NullPointerException {
        ...
        if (refreshToken == null) {
            ...
            if (verifyToken(refreshToken) == true) {
                // Determine email and accountFrom from DTO if available, otherwise from
                // existing logic (which was missing!)
                // Wait, the original code used 'person' request body but it was likely
                // empty/wrongly mapped?
                // Actually verifyToken just returns true. We need to extract claims.
                // The original code passed @RequestBody Person person. But we might need to
                // extract 'email' and 'accountFrom' from the JWT itself if not provided?
                // Or we can trust the requestBody providers.
                ...
                if (requestBody != null) {
                    ...
                    accountFrom = requestBody.getAccountFrom();
                } else {
                    // Fallback to extracting from token if possible, or error.
                    // Original code relied on Person passed in body. Let's keep supporting that
                    // logic via DTO.
                    SignedJWT signedJWT = SignedJWT.parse(refreshToken);
                    email = signedJWT.getJWTClaimsSet().getSubject();
                    // AccountFrom is not in standard claims, might be tough.
                    // The original code required Frontend to send Person object.
                    // My DTO has email and accountFrom.
                }

                // Let's rely on extracting from Token if possible, or DTO.
                if (email == null) {
                    SignedJWT signedJWT = SignedJWT.parse(refreshToken);
                    email = signedJWT.getJWTClaimsSet().getSubject();
                    // Assume default provider if missing? Or error.
                    if (accountFrom == null)
                        accountFrom = "APP_LOG"; // Default local
                }
                ...
                if (user.isPresent()) {
                    ...
                    if (checkedRemember == true) {
                        response.addHeader("Set-Cookie",
                                "JWToken=" + token + "; Max-Age=" + maxAge + "; Path=/; HttpOnly");
                        response.addHeader("Set-Cookie", "RefreshJWTToken=" + refreshTokenReponse + "; Max-Age="
                                + maxAgeRefresh + "; Path=/; HttpOnly");
                    } else {
                        response.addHeader("Set-Cookie", "JWToken=" + token + "; Path=/; HttpOnly");
                        response.addHeader("Set-Cookie", "RefreshJWTToken=" + refreshToken + "; Path=/; HttpOnly");
                    }

                    // ALSO return JSON body with new tokens for Cookieless support
                    try {
                        response.setContentType("application/json");
                        String jsonResponse = String.format(
                                "{\"token\": \"%s\", \"refreshToken\": \"%s\", \"authenticated\": true}", token,
                                refreshTokenReponse);
