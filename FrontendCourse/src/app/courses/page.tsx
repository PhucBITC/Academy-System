...
const coursesPage = () => {
    ...
    const fetchCoursesFilter = async (rating: string | null, languages: string | null, categories: string | null, levels: string | null,
        ...
        try {
            ...
            // Client-side strict filtering to create buckets
            let finalCourses = coursesWithRatings;
            if (rating) {
                const ratingValue = parseFloat(rating);
                if (ratingValue === 4.0) {
                    finalCourses = finalCourses.filter(item => item.rating < 4.5);
                } else if (ratingValue === 3.5) {
                    finalCourses = finalCourses.filter(item => item.rating < 4.0);
                } else if (ratingValue === 3.0) {
                    finalCourses = finalCourses.filter(item => item.rating < 3.5);
                }
            }

            setDataCourseFilter(finalCourses);
